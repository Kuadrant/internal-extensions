...
func main() {
	reconciler := controller.NewPipelinePolicyReconciler()
	builder, logger := extcontroller.NewBuilder("pipeline-policy-extension-controller")
	extController, err := builder.
		WithScheme(scheme).
		WithReconciler(reconciler.Reconcile).
		For(&v1alpha1.PipelinePolicy{}).
		Build()
	if err != nil {
